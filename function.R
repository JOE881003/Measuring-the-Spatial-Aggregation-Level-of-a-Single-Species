...
CECI <- function(mat, area, method = "P")#參數method控制是否使用平行運算，"P"使用平行運算，"NP"不使用平行運算
{
...
#########roll data(將一維資料前或後移動n次，可用於計算離前n個個體或後n個個體的距離，本篇只前後分別移動一次，計算每個個體前一個與後一個的距離，也就是論文中的w_{aj}與w_{bj})
...
#########Find nearest neighbor distance in line(在一維資料中，找到每個個體的最近鄰距離的平均，也就是論文中的\bar{l}_{Aj})
NN_1d <- function(data){
  n = length(data)
  if(n > 1){
  data <- sort(data)#排序資料
...
#########One dimension NN(結合最近鄰距離的平均值與期望值，構成的一維最近鄰指標，輸出結果包含一維最近鄰指標, 最近鄰距離平均值, 最近鄰距離期望值, 最近鄰距離標準誤, p-value)
NNL <- function(data, L){
  n <- length(data)
  data <- sort(data)
  ra <- NN_1d(data)
...
#########NN index use C(使用套件spatialEco中的nni函數進行最近鄰指標C語言運算，這邊我們多賦予這個函數可以給定面積的功能，參數為"A"，預設值為1000*500)
...
#########QNN use C(使用套件spatstat中的nndist函數計算個體的最近鄰距離，nndist預設使用C語言計算) 
...
########Calculate traditional NN index, but use quadrat data(將研究區域切割成多個帶狀區塊，並計算最近鄰指標，之後加權平均)
